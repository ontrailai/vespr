...
export function FinalCtaSection() {
  ...
  return (
    <section 
      ...
      style={{
        background: "linear-gradient(135deg, #0a0a0a 0%, #1a1030 50%, #0f0f1a 100%)"
      }}
    >
      {/* Purple glow effects */}
      <div 
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[600px] opacity-30 blur-3xl pointer-events-none"
        style={{ background: "radial-gradient(ellipse, #6b46c1 0%, transparent 70%)" }}
      />
      <div 
        className="absolute bottom-0 right-0 w-[400px] h-[400px] opacity-20 blur-3xl pointer-events-none"
      ...
      <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 text-center">
        ...
        {/* Headline */}
        <h2 
          className={`font-serif text-4xl sm:text-5xl lg:text-6xl font-bold text-white leading-tight transition-all duration-700 delay-100 ${
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
          }`}
        >
        ...
        >
          <Button 
            asChild 
            size="lg" 
            className="rounded-full px-10 py-7 text-lg font-semibold text-white shadow-2xl shadow-purple-500/25 transition-all hover:-translate-y-1 hover:shadow-purple-500/40"
            style={{
        ...
        {/* Guarantee */}
        <p 
          className={`mt-4 text-sm font-medium text-emerald-400 transition-all duration-700 delay-600 ${
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
          }`}
        >
